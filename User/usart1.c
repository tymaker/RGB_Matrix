...
/*
 * 函数名：USART1_Config
 * 描述  ：USART1_GPIO 配置，工作模式配置。11500 8-N-1
 * 输入：void
 * 输出：void
 * 调用：外部调用
 */
...
void USART1_Config(void)
{
    GPIO_InitTypeDef GPIO_InitStructure;
	  ...
	  /* USART1 GPIO config */
	  /* Config USART1 Tx (PB06) as alternate function push-pull */
	  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;
	  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(GPIOB, &GPIO_InitStructure);
	  /* Configure USART1 Rx (PB07)  as input floating */
...
//    USART_ITConfig(USART1,USART_IT_RXNE,ENABLE);               //允许串口1接收中断  接收缓存为空
